...
from api.services import DataSourceService as _DataSourceService, DataSourceTokenService as _DataSourceTokenService
...
create_data_source_dto = api.model(
    'CreateDataSource', {
        'source': fields.String(description="description of the data source",
    ...
    def post(self, **kwargs):
        ...
        try:
            return jsonify(
                SuccessObject.create_response(
                    self, HTTPStatus.OK,
                    _data_source_service.add_data_source(self, kwargs['tupled_output'], token['user']['id'])))
        except Exception as err:
            return ErrorObject.create_response(self, err.args[0], err.args[1])
